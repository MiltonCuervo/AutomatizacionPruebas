...
public class FillMasterCreditDetails implements Interaction {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                WaitUntil.the(CheckoutPageElements.INPUT_CARD_NUMBER, isPresent())
                        .forNoMoreThan(10).seconds(),

                Enter.theValue(cardNumber)
                        .into(CheckoutPageElements.INPUT_CARD_NUMBER),

                Enter.theValue(cvv)
                        .into(CheckoutPageElements.INPUT_CVV),

                SelectFromOptions.byVisibleText(expiryMonth)
                        .from(CheckoutPageElements.SELECT_EXPIRY_MONTH),

                SelectFromOptions.byVisibleText(expiryYear)
                        .from(CheckoutPageElements.SELECT_EXPIRY_YEAR),

                Enter.theValue(cardholderName)
                        .into(CheckoutPageElements.INPUT_CARDHOLDER_NAME)
        );
    }
}